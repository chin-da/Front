...
const Select = memo(
  forwardRef<HTMLDivElement, SelectProps>(
    ({ value = '', onValueChange, placeholder, children, ...restProps }) => {
      const refs = useRef<HTMLDivElement | null>(null);
      const findValueOfTextObj = Children.toArray(children).find(
        (e: ReactNode) => {
      ...
      return (
        <SelectContext.Provider value={context}>
          <SelectWrapper
            ...
            className="select-SelectWrapper"
            {...restProps}
            ref={refs}
          >
            <StyledSelect>{text || placeholder}</StyledSelect>
            {isOpen && (
              <StyledSelectItemBox className="select-SelectItemBox">
                {children}
              </StyledSelectItemBox>
            )}
            <StyledIcon>
...
const SelectItem = memo(
  forwardRef<HTMLButtonElement, SelectItemProps>(
    ({ prefix, suffix, text, value, ...restProps }, ref) => {
      ...
      return (
        ...
        >
          <StyledSelectItem
            className={`select-SelectItem ${isSelected ? 'selected' : ''}`}
          >
            {typeof prefix !== undefined && (
              <StyledDecorateFix>{prefix}</StyledDecorateFix>
            )}
            <Text>{text}</Text>
            {typeof suffix !== undefined && (
              <StyledDecorateFix>{suffix}</StyledDecorateFix>
            )}
