...
const ThumbnailOverlayContext =
  createContext<ThumbnailOverlayContextProps | null>(null);
const ThumbnailOverlay = memo(
  forwardRef<HTMLDivElement, ThumbnailOverlayProps>(
    ({ overlay, children, ...restProps }, ref) => {
      const overlayState = useState(overlay);
      const isOpenState = useState(true); //초기 value가 children obj에 없으면 초기화시킨다.
      const context = useMemo(
        () => ({ overlayState, isOpenState }),
        [overlayState, isOpenState],
      );
      return (
...
const Thumbnail = memo(
  forwardRef<HTMLImageElement, ThumbnailThumbnailProps>(
    ({ src, currentTarget, ...restProps }, ref) => {
...
const Close = memo(
  forwardRef<HTMLDivElement, ThumbnailCloseProps>(
